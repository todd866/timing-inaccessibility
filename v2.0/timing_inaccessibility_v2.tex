...
\textbf{Step 1:} Define coarse-grained entropy as $S_\varepsilon = \kB(\ln N_\varepsilon - \mathrm{KL}(p\|U))$ where $U$ is uniform over the $N_\varepsilon$ cells. \emph{Note:} Identifying $S_\varepsilon$ with a thermodynamic entropy that enters a Clausius inequality requires equilibrium or near-equilibrium (linear response) conditions. For far-from-equilibrium systems, $S_\varepsilon$ serves as an information-theoretic proxy; the bound remains valid as stated when $\Teff$ is the operationally measured noise--response ratio at the collapse mode (see validity conditions below).
...
Bormashenko and Voronel \cite{voronel2018} and Bormashenko \cite{bormashenko2022} established that biological informational capacity is constrained by spatial scales. Biological systems exploit \emph{both} spatial and temporal fine-graining. Total capacity:
\begin{equation}
...
A philosophical consequence of framework-dependent timing: below the registration threshold, temporal order is not a physically well-defined, testable property of the system. The coarse-grained order that appears at projection is \emph{created} by the measurement framework, not revealed. Below the Landauer threshold, the ordering is operationally undecidable---there is no way to access the ``true'' micro-ordering without spending energy that exceeds the signal. Causal relationships inherit this framework dependence: what counts as ``A caused B'' depends on the temporal resolution chosen before measurement.
