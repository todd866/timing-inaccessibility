...
\begin{abstract}
Maxwell's demon was resolved by Landauer and Bennett: the demon must erase information to reuse its memory, paying $\ge \kB T\ln 2$ per bit. That resolution, however, is usually phrased for discrete memory states. Here we extend the discussion to continuous biological substrates. Our central claim is that the energetic advantage of living systems arises from \emph{timing inaccessibility}: irreversibly registering fine temporal order at resolution $\Delta t$ requires dissipating $\ge \kB T\ln 2$ per stabilized order bit, and when the relevant couplings are sub-Landauer, obtaining such records without dominating the dynamics demands measurement resources that overwhelm the coupling energies (back-action). Exponentially many micro-trajectories then map to the same observable outcome (\emph{path degeneracy}), allowing continuous high-dimensional substrates to integrate weak couplings during evolution and pay primarily at \emph{projection} (dimensional collapse to a low-dimensional output).

We derive a coarse-grained \textbf{Projection Bound} for quasistatic collapse at effective temperature $\Teff$:
\begin{equation}
...
\right)
\end{equation}
which reduces under typical-set conditions to $E_{\mathrm{collapse}}\ge \kB\Teff\ln(N_{\varepsilon,\mathrm{pre}}/N_{\varepsilon,\mathrm{post}})$. Combined with a \textbf{Temporal Registration Bound} (ordering $M$ bins requires $\log_2 M!$ bits), this yields a simple architectural contrast: digital tracking pays at each stabilized record, whereas continuous substrates defer most of the thermodynamic payment to projection.

\textbf{Version 2.0} clarifies the two-stage model (reversible correlation followed by irreversible projection), discusses temporal resolution as part of the measurement framework \cite{todd2025biosystems}, and examines how the placement of the projection boundary constrains artificial systems.
...
\textbf{Path Degeneracy Scaling (heuristic estimate):} With coherence time $\tau_c$ and temporal resolution $\Delta t$, assuming independent modes with correlation correction $\kappa$:
\begin{equation}
\Omega(\Delta t) \sim \exp\!\left[\frac{D_{\mathrm{eff}}(\Delta t)}{\kappa}\,\ln\!\frac{\tau_c}{\Delta t}\right] \label{eq:omega}
\end{equation}
where $\kappa \ge 1$ accounts for correlations. Unlike the TRB and PB above, this is a scaling estimate under independence assumptions, not a proven bound.
}}\medskip
...
\item Energy dissipated during tracking: $E_{\text{digital}} \gtrsim (160\text{--}332) \kB T\ln 2 \approx 4.6\times10^{-19}$--$9.5 \times 10^{-19}$ J (ideal Landauer floor; practical CMOS is typically far higher, while fully reversible logic can approach the floor only with severe speed and noise-tolerance tradeoffs \cite{bennett1982,horowitz2014})
...
\textbf{Efficiency gain:} $\sim 160$--$330\times$ at the theoretical Landauer limit. In practice the gap is much larger because contemporary digital hardware operates well above that floor \cite{horowitz2014}. Reversible computing can, in principle, narrow the gap \cite{bennett1982}, but only with substantial speed and noise-tolerance tradeoffs. Digital architectures pay per recorded bit; continuous substrates defer recording and pay at projection. Analog and neuromorphic systems gain efficiency to the extent that they also defer discretization.
...
Section 2 formalizes timing inaccessibility. Section 3 derives the Projection Bound. Section 4 quantifies path degeneracy. Section 5 gives a continuous (Kuramoto) example. Section 6 relates to prior work and discusses implications including camera-engine duality. \textbf{Version 2.0 additions:} Section 7 establishes framework dependence of timing (including the emergent character of temporal order). Section 8 derives implications for AI capability limits. Appendix A provides parameter sensitivity analysis for degeneracy estimates. Appendix B describes biological implementations across scales. Appendix C provides methods for estimating $D_{\mathrm{eff}}$ and $\kappa$ from data.
...
\textbf{Scope:} The following applies to architectures where all internal state transitions involve irreversible bit stabilization. Systems with analog front-ends or hybrid architectures can shift the projection boundary outward, accessing some sub-Landauer integration (see \S\ref{sec:neuromorphic}).
...
\textbf{Step 1:} Define coarse-grained entropy as $S_\varepsilon = \kB(\ln N_\varepsilon - \mathrm{KL}(p\|U))$ where $U$ is uniform over the $N_\varepsilon$ cells. \emph{Note:} Identifying $S_\varepsilon$ with a thermodynamic entropy that enters a Clausius inequality requires equilibrium or near-equilibrium (linear response) conditions. For far-from-equilibrium systems, $S_\varepsilon$ serves as an information-theoretic proxy; in that regime the bound should be understood as an \emph{operational ansatz} calibrated by the measured noise--response ratio $\Teff(\omega_0)$, not as a strict thermodynamic inequality (see validity conditions below).
...
This is the continuous analog of Landauer's bound---fully geometric, no discrete bits. Bian et al.\ \cite{bian2025ddga} observe numerical ``covariance explosion'' in high-dimensional oscillatory systems at fine temporal resolution, which is consistent with---though not direct evidence for---the thermodynamic bounds derived here.
...
Bormashenko and Voronel \cite{voronel2018} and Bormashenko \cite{bormashenko2022} established that biological informational capacity is constrained by spatial scales. When the system's attractor has both spatial degrees of freedom ($D_{\text{spatial}}$, at resolution $\xi$) and temporal degrees of freedom ($D_{\text{temporal}}$, at resolution $\Delta t$), the total covering number factorises: $N_\varepsilon \sim (L/\xi)^{D_{\text{spatial}}} \cdot (\tau/\Delta t)^{D_{\text{temporal}}}$. Applying the Projection Bound (Theorem~\ref{thm:projection-bound}) to the combined manifold:
\begin{equation}
...
\textbf{The biological demon is more subtle.} It is a high-dimensional dynamical system that simultaneously functions as both \textbf{camera} (sensing environmental complexity) and \textbf{engine} (steering environmental dynamics). The duality is simultaneous, with thermodynamic cost concentrated at the decision/collapse point.
...
The demon \emph{becomes isomorphic} to environmental structure via dimensional resonance rather than writing bits. Information exists in the physical \emph{correlation} between internal and external structure (coupling energy), and thermodynamic cost appears only when correlation collapses to discrete action.

High-$D$ systems maintain mutual information through structural isomorphism without irreversible bit registration. A protein's $10^{48}$ microstates resonantly couple to chaperone conformations; the mutual information exists in physical correlation. Dimensional collapse occurs only when the system commits to the native fold.
...
The limitation is physical, not computational. Temporal structure is created by registration, not revealed, so any predictor must perform the same registration operations---and therefore emulate the system up to collapse points. In Igamberdiev's framework \cite{igamberdiev2014}, biological organization involves anticipatory systems where constraints precede values; camera-engine duality enables this by maintaining correlations without computing trajectories.
...
Events that are ``sequential'' for Observer A may be ``simultaneous'' for Observer B. The two observers inhabit different causal worlds: A causes B for Observer A; A and B co-occur for Observer B.
...
The underlying physics exists independently of frameworks, but \emph{causal claims are framework-relative}, and the framework cannot be tested from within.
...
A philosophical consequence of framework-dependent timing: below the registration threshold, temporal order is not operationally recoverable without back-action that exceeds the signal energy. The coarse-grained order that appears at projection is framework-relative---different measurement budgets yield different orderings of the same physical events, and there is no budget-independent way to adjudicate between them. Causal relationships inherit this framework dependence: what counts as ``A caused B'' depends on the temporal resolution chosen before measurement.
...
As in \S2.4, the following applies to conventional digital architectures with irreversible bit stabilization at each stage; hybrid architectures shift the boundary (\S\ref{sec:neuromorphic}). Conventional digital computers are committed to supra-Landauer discretization at the input boundary, with three consequences:
...
\textbf{Turing completeness vs.\ thermodynamic accessibility.} Digital systems can compute any computable function given sufficient time and memory, but purely digital internal state evolution cannot exploit sub-Landauer integration without shifting the projection boundary into analog/hybrid components (\S\ref{sec:neuromorphic}). The constraint is physical, not computational: simulation can reproduce the output but not the thermodynamic shortcut.

\subsection{What Conventional Digital AI Cannot Do}

If the sub-Landauer integration regime is essential for certain biological computations, then conventional digital AI cannot exploit the same thermodynamic shortcut without analog or hybrid front-ends. This is a claim about \emph{mechanism and efficiency}, not about input-output equivalence---a digital system may approximate the same behavior via different (and potentially costlier) means. Candidates for mechanism-dependent differences include:
...
The efficiency gain reflects access to a computational regime that purely digital systems cannot reach:
...
\bibitem{voronel2018}
Bormashenko, E., Voronel, A. (2018). Spatial scales of living cells and their energetic and informational capacity. \textit{Eur. Biophys. J.}, 47, 515--521. doi: 10.1007/s00249-017-1267-x

\bibitem{bormashenko2022}
Bormashenko, E. (2022). Fibonacci sequences, symmetry and order in biological patterns, their sources, information origin and the Landauer principle. \textit{Biophysica}, 2(3), 292--307. doi: 10.3390/biophysica2030027
...
\bibitem{frank2019}
Frank, M.P. (2002). The physical limits of computing. \textit{Computing in Science \& Engineering}, 4(3), 16--26. doi: 10.1109/5992.998637
